...
DEX_TO_JAR_TOOL = 'E:/WORK/PY_PROJECT/TOOL/quickapktool/libs/dex2jar/d2j_invoke.bat'
...
def convert_dex_to_jar(dexdir):
    ...
    for dex_file in os.listdir(input_dir):
        if dex_file.lower().endswith('.dex'):
            ...
            subprocess.run([DEX_TO_JAR_TOOL, 'com.googlecode.dex2jar.tools.Dex2jarCmd',
                            os.path.join(input_dir, dex_file), '--force', '-o', jarfile], shell=True)
            jars.append(jarfile)
...
def runcore(project_dir):
    print('-----------')
    st = time.time()
    convert_smali_to_dex(project_dir)
    convert_dex_to_jar(project_dir)
    print('done after:' + str(time.time() - st) + 's')
